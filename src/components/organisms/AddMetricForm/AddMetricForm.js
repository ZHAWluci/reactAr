...
const AddMetricForm = (props) => {
  ...
  const enteredMetricTitleIsValid =
    enteredMetricTitle.trim() !== "" && !enteredMetricTitleExists;
  const enteredMetricTitleHasError =
    !enteredMetricTitleIsValid && enteredMetricTitleTouched;
  ...
  const clickAddHandler = () => {
    ...
    if (
      cards[activeCardIndex].metrics
        .map((metric) => metric.metricTitle === enteredMetricTitle)
        .includes(true)
    ) {
      ...
      <div className="metric-form__add">
        <input
          ...
          className={`metric-form__add-input ${
            enteredMetricTitleHasError && "metric-form__add-input--error"
          }`}
        ...
        />
        {!enteredMetricTitleExists && (
          <label
            htmlFor="metric-form__add-input"
            className="metric-form__add-label"
          >
            Metric Name
          </label>
        )}
        {enteredMetricTitleExists && (
          <p className="metric-form__add-error">
            This Metric Name already exists
          </p>
        )}
        <button onClick={clickAddHandler} className="metric-form__add-button">
          Add
        </button>
      </div>
    </MetricsWrapper>
